...
function nodeMouseOver(event, d){
    ...
    try {
        ...
        messages="国家："+result[0].Country+"幸福指数"+result[0].HappinessScore;
...
const colorScale = d3.scaleThreshold()
    .domain([100,250, 350, 450, 550, 600, 700])
    .range(d3.schemeBlues[6])
d3.csv( "../2017.csv" ).then (data => {
    ...
    svg.append("g")
        ...
        .attr("fill", function (d) {
          //  d.total = data.get(d.id) || 0;
            try {
                let result = dataone.filter(word => word.Country==d.properties.name);
                return colorScale(result[0].HappinessScore*100);
            } catch(err){
